...
def env(**kwargs):
    ...
    """
    env = raw_env(**kwargs)
    # this wrapper helps error handling for continuous action spaces
...
class parallel_env(ParallelEnv, EzPickle):
    ...
    def __init__(
        ...
        self.action_spaces = dict(
            zip(
                self.possible_agents,
                [Box(low=np.array([-speed_max, -psidot_max]), high=np.array([speed_max, psidot_max]), dtype=np.float64)]
                * self.n_vehicles,
            )
        )
    ...
    def is_static_action(self, action: [float, float]) -> bool:
        """

...
class PythonMsg:
    ...
    def __setattr__(self, key, value):
        ...
        if not hasattr(self, key):
            raise TypeError(
                'Cannot add new field "%s" to frozen class %s' % (key, self)
            )
        else:
    ...
    def from_str(self, string_rep):
        ...
        for key in vars(self):
            val_str_index = (
                string_rep.find(key + "=", val_str_index) + len(key) + 1
            )  # add 1 for the '=' sign
            value_substr = string_rep[
                val_str_index : string_rep.find(",", val_str_index)
            ]  # (thomasfork) - this should work as long as there are no string entries with commas
...
class OrientationQuaternion(PythonMsg):
    ...
    def from_yaw(self, yaw):
        ...
        if abs(yaw - self.to_yaw()) % (2 * np.pi) > 1e-9:
            pdb.set_trace()
...
class VehicleState(PythonMsg):
    ...
    q: OrientationQuaternion = field(
        default=None
    )  # global orientation (qr, qi, qj, qk)
    ...
    def __post_init__(self):
        if self.x is None:
            self.x = Position()
        if self.u is None:
            self.u = VehicleActuation()
        if self.v is None:
            self.v = BodyLinearVelocity()
        if self.w is None:
            self.w = BodyAngularVelocity()
        if self.a is None:
            self.a = BodyLinearAcceleration()
        if self.aa is None:
            self.aa = BodyAngularAcceleration()
        if self.q is None:
            self.q = OrientationQuaternion()
        if self.e is None:
            self.e = OrientationEuler()
        if self.p is None:
            self.p = ParametricPose()
        if self.pt is None:
            self.pt = ParametricVelocity()
        return
...
class VehiclePrediction(PythonMsg):
    ...
    psidot: array.array = field(
        default=None
    )  # global and local angular velocity of car

    v_long: array.array = field(
        default=None
    )  # longitudinal velocity (in the direction of psi)
    v_tran: array.array = field(
        default=None
    )  # transverse velocity   (orthogonal to the direction of psi)

    a_long: array.array = field(
        default=None
    )  # longitudinal velocity (in the direction of psi)
    a_tran: array.array = field(
        default=None
    )  # transverse velocity   (orthogonal to the direction of psi)
    ...
    s: array.array = field(
        default=None
    )  # path length along center of track to projected position of car
    x_tran: array.array = field(
        default=None
    )  # deviation from centerline (transverse position)

    u_a: array.array = field(default=None)  # acceleration output
    u_steer: array.array = field(default=None)  # steering angle output
    u_steer_dot: array.array = field(default=None)  # angular velocity of steering angle
    ...
    local_state_covariance: array.array = field(
        default=None
    )  # Vectorized upper triangular part of covariance matrix with main diagonal order [v_long, v_tran, psidot, e_psi, s, e_y]
    global_state_covariance: array.array = field(
        default=None
    )  # Vectorized upper triangular part of covariance matrix with main diagonal order [x, y, psi, v_long, v_tran, psidot]

    def update_body_velocity_from_global(self):
        self.v_long = (
            np.multiply(self.v_x, np.cos(self.psi))
            + np.multiply(self.v_y, np.sin(self.psi))
        ).tolist()
        self.v_tran = (
            -np.multiply(self.v_x, np.sin(self.psi))
            + np.multiply(self.v_y, np.cos(self.psi))
        ).tolist()
        self.a_long = (
            np.multiply(self.a_x, np.cos(self.psi))
            + np.multiply(self.a_y, np.sin(self.psi))
        ).tolist()
        self.a_tran = (
            -np.multiply(self.a_y, np.sin(self.psi))
            + np.multiply(self.a_y, np.cos(self.psi))
        ).tolist()

    def update_global_velocity_from_body(self):
        self.v_x = (
            np.multiply(self.v_long, np.cos(self.psi))
            - np.multiply(self.v_tran, np.sin(self.psi))
        ).tolist()
        self.v_y = (
            np.multiply(self.v_long, np.sin(self.psi))
            + np.multiply(self.v_tran, np.cos(self.psi))
        ).tolist()
        self.a_x = (
            np.multiply(self.a_long, np.cos(self.psi))
            - np.multiply(self.a_tran, np.sin(self.psi))
        ).tolist()
        self.a_y = (
            np.multiply(self.a_long, np.sin(self.psi))
            + np.multiply(self.a_tran, np.cos(self.psi))
        ).tolist()
